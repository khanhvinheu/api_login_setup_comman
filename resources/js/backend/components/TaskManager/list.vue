<template>
    <div class="row">
        <div class="col-md-12">
            <div class="card card-default">
                <div class="card-header" style="background-color: rgb(0,0,0,0.1);">
                    <h3 class="card-title">DANH SÁCH CÔNG VIỆC</h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                        </button>
                        <!-- <button type="button" class="btn btn-tool" data-card-widget="remove">
                          <i class="fas fa-times"></i>
                        </button> -->
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row" style="display: flex; flex-wrap: nowrap; padding: 8px; justify-content: space-between">
                                <el-input
                                    style="width: 500px"
                                    v-model="textSearch"
                                    placeholder="Nhập kí tự cần tìm kiếm ..."
                                    @keyup.enter.native="getList()"
                                >
                                    <template v-slot:append>
                                        <el-button type="primary" @click="getList()"><i class="el-icon-search"></i> Tìm
                                            kiếm
                                        </el-button>

                                    </template>
                                </el-input>
                                <el-button @click="$router.push({name:'TaskCreate'})" class="ml-2" type="primary"><i
                                    class="el-icon-plus"></i> Thêm mới
                                </el-button>
                            </div>
                            <el-table
                                empty-text="Chưa có dữ liệu !"
                                :data="tableData"
                                style="width: 100%"
                                border
                                @expand-change="changeRow"
                                :resizable="true"
                                v-loading="loadingTable"
                                :row-class-name="tableRowClassName">
                                <el-table-column
                                    prop="title"
                                    label="TITLE"
                                    width="200"
                                    sortable
                                >
                                </el-table-column>
                                <el-table-column
                                    prop="task_type"
                                    label="LOẠI CÔNG VIỆC"
                                >
                                    <template slot-scope="scope">
                                        <span>{{ scope.row.task_type[0].title}}</span> 
                                    </template> 
                                </el-table-column>
                                <el-table-column
                                    prop="assign_to"
                                    width="150"
                                    label="GIAO CHO"
                                >
                                    <template slot-scope="scope">
                                        <el-tag>{{ scope.row.assign_to[0].name}}</el-tag> 
                                    </template>           
                                </el-table-column>
                               
                                <el-table-column
                                    prop="create_by"
                                    width="150"
                                    label="NGƯỜI TẠO"
                                >
                                    <template slot-scope="scope">
                                        <el-tag>{{ scope.row.create_by[0].name}}</el-tag> 
                                    </template>                    
                                </el-table-column>
                                <el-table-column
                                    prop="progress"
                                    width="100"
                                    label="TIẾN ĐỘ"
                                >
                                    <template slot-scope="scope">
                                        <el-progress :stroke-width="10" :width="50" type="dashboard" :percentage="parseInt(scope.row.progress) " :color="colors"></el-progress>
                                        
                                    </template>                    
                                </el-table-column>
                                <el-table-column
                                    prop="status"
                                    width="150"
                                    label="TRẠNG THÁI"
                                >
                                    <template slot-scope="scope">
                                        <el-tag :type="listStatus.find(e=>e.value == scope.row.status).type">
                                            <span v-html="listStatus.find(e=>e.value == scope.row.status).title"></span>    
                                        </el-tag> 
                                    </template>                    
                                </el-table-column>
                               

                                <el-table-column
                                    prop="created_at"
                                    label="NGÀY TẠO"
                                    width="150"
                                >
                                    <template slot-scope="scope">
                                        {{ scope.row.created_at | formatDate}}
                                    </template>
                                </el-table-column>  
                                     
                                <el-table-column
                                    label="THAO TÁC"
                                    width="180"
                                >
                                    <template slot-scope="scope">
                                        <el-button  @click="update(scope.row)" type="primary" icon="el-icon-edit" circle></el-button>
                                        <el-button  @click="detail(scope.row)" type="info" icon="el-icon-message" circle></el-button>
                                        <el-popconfirm
                                            confirm-button-text='Xóa'
                                            cancel-button-text='Không'
                                            :title="'Bạn có chắc chắn muốn xóa?'"
                                            @confirm="()=>deleteUser(scope.row.id)"
                                        >
                                        <el-button style="margin-left: 10px;" slot="reference" type="danger" icon="el-icon-delete" circle></el-button>
                                        </el-popconfirm>
                                    </template>
                                </el-table-column>
                                <template slot="empty">
                                    <el-empty description="No data"></el-empty>
                                </template>
                            </el-table>
                        </div>
                        <div class="block" style="margin-left: 0px;margin-right: 8px;padding: 10px;width: 100%">
                            <el-pagination
                                @size-change="handleSizeChange"
                                @current-change="handleCurrentChange"
                                :current-page.sync="currentPage"
                                :page-sizes="[10, 20, 50, 100]"
                                :page-size="options.PageLimit"
                                layout="total, sizes, prev, pager, next, jumper"
                                :total="options.Total">
                            </el-pagination>
                        </div>
                        <!-- /.col -->

                    </div>
                    <!-- /.row -->
                </div>
                <!-- /.card-body -->
                <div class="card-footer">

                </div>
            </div>
        </div>       
    </div>

</template>

<script>
import { listStatus } from './configStatus';
export default {   
    data() {
        return {
            colors: [
                {color: '#f56c6c', percentage: 20,status:"exception"},
                {color: '#6f7ad3', percentage: 40},
                {color: '#e6a23c', percentage: 60},
                {color: '#1989fa', percentage: 80},
                {color: '#5cb87a', percentage: 100,status:"success"}
            ],
            idUpdate:'',
            outerVisible:false,
            loadingTable:false,
            tableData: [],
            slideData: [],
            textSearch: '',
            currentPage: 1,
            options:{
                Total:10,
                Page:1,
                PageLimit:10
            },
            triggerLoad:new Date().getTime(),
            listStatus:listStatus
           
        }
    },
    mounted() {
        this.getList()
    },
    methods: {
        changeRow(e){
            console.log(e);
        },
        success(){
          this.outerVisible = false
          this.getList()
        },
        update(e){
            this.idUpdate = e.id
            this.$router.push({name:'TaskUpdate',params:{id:e.id}})
        },
        detail(e){
            this.idUpdate = e.id
            this.$router.push({name:'TaskDetail',params:{id:e.id}})
        },
        handleSizeChange(val) {
            this.options.PageLimit = val
            this.getList()
        },
        handleCurrentChange(val) {
            this.options.Page = val
            this.getList()
        },
        updateStatus(id, hidden) {
            let _this = this
            var formData = new FormData()
            console.log(hidden, hidden == "0" ? "1" : "0")
            formData.append('hidden', hidden == "0" ? "1" : "0")
            axios({
                method: 'post',
                url: '/api/admin/product_color/update/' + id,
                data: formData
            })
                .then(function (response) {
                    if (response.data['success']) {
                        _this.$notify({
                            title: 'Success',
                            message: response.data['mess'],
                            type: 'success'
                        });
                        _this.getList()
                    } else {
                        _this.$notify({
                            title: 'Error',
                            message: response.data['mess'],
                            type: 'error'
                        });
                    }

                });

        },
        deleteUser(id) {
            let _this = this
            axios({
                method: 'post',
                url: '/api/admin/list-task/delete/' + id,
            })
                .then(function (response) {
                    if (response.data['success']) {
                        _this.$notify({
                            title: 'Success',
                            message: response.data['mess'],
                            type: 'success'
                        });
                        _this.getList()
                    } else {
                        _this.$notify({
                            title: 'Error',
                            message: response.data['mess'],
                            type: 'error'
                        });
                    }
                });
        },
        getList() {
            let _this = this
            _this.loadingTable = true
            let param = {}
            this.options.Page &&(param.page = this.options.Page)
            this.options.PageLimit &&(param.limit = this.options.PageLimit)
            this.textSearch && (param.TextSearch = this.textSearch)
            axios({
                method: 'get',
                url: '/api/admin/list-task',
                params: param
            })
                .then(function ({data}) {
                    if (data['success']) {
                        _this.tableData = data['data']
                        _this.options.Total = data['count']
                 
                    }
                    _this.loadingTable = false
                });
        }, changeStatus(id) {
            alert(id)
        },
        tableRowClassName({row, rowIndex}) {
            if (rowIndex === 1) {
                return 'warning-row';
            } else if (rowIndex === 3) {
                return 'success-row';
            }
            return '';
        }
    }
};
</script>

<style>
.el-table .warning-row {
    background: oldlace;
}

.el-table .success-row {
    background: #f0f9eb;
}
.color-item{
    height: 20px;
    width: 40px;
    border-radius: 6px;
}
</style>
